...
目前可啟動前後端、查看東京三日行程、開啟 Google Maps，並以 LLM structured output（失敗時使用本機 fallback）解析多日事件。Backend A 已接好 Backend B 的 `LLMReplanner`，Replan 會回傳 live 或離線 fixture 的 A/B/C `ready` 方案；ready snapshot、`POST /api/selections` 原子套用與偏好學習亦已完成。推薦文字目前使用 deterministic fallback，規格中的 LLM 推薦說明仍待實作。
...
`.env` 放在 repo 根目錄，由 Agent 與 Backend B 共用 `LLM_` 設定；安全範例見 `.env.example`。設定 `LLM_API_KEY` 與 `LLM_MODEL` 後，A 會用 Chat Completions strict JSON Schema 解析事件，B 會用 Responses API 產生重排方案；未設定 key 時 B 使用明確標示的離線 planning fixture，A 則使用本機 parser 並加入 warning。事件 parser 的 context 僅包含訊息、行程日期／時區及活動 ID／名稱／日期，不包含座標、預約、偏好或 runtime 歷史。推薦文字目前由 deterministic fallback 產生，尚未把方案摘要送往 LLM。`LLM_TIMEOUT_SECONDS` 預設 8 秒；key 僅供後端使用，勿提交或放入 VITE\_ 變數。`WEATHER_MODE=mock` 使用涵蓋 Trip 未來日期區間、明確標示來源的天氣 fixture；`live` 呼叫 Open-Meteo date range 並在失敗時標示 fallback。
...
後端 API schema 已固定為多日：Trip 有 `version`、`start_date`／`end_date`，每個 TripItem 有 `scheduled_date`，Event 使用 arrays，Plan 包含 changes/impact/features，ReplanResponse 包含 snapshot identity 與 recommendation，SelectionResponse 回傳 selection、更新後 Trip 與 Preference。`frontend/` 已同步契約並依日期分組呈現。

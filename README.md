...
`.env` 放在 repo 根目錄，由後端明確載入；初始化不需要 key。未來 LLM key/model 僅供後端使用，勿放入 VITE\_ 變數。WEATHER_MODE 預留 mock/live 設定，目前 placeholder 不使用外部天氣。種子資料在 backend/data/；runtime 寫入 backend/data/runtime/ 或 SQLite，已忽略版控。

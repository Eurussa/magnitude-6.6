...
Accepted — 2026-09-12（外層 contract 已固定；snapshot、選擇交易與偏好更新仍待實作）
...
- Snapshot 驗證、SelectionRecord 保存、Plan 套用、Trip.version +1 與 Preference 更新必須在同次 runtime 原子交易完成。相同選擇重送冪等回 200且不重複加分；同次改選、不可行或 stale 回 409；snapshot/plan 不存在回 404；目前未實作時回 501；storage error 回 503。
- Preference 三權重初始皆 1；選擇後對所選策略 +1，增加 selection_count。B 用 `sum(weight * feature)` 排序，同分固定 A/B/C；這不是模型訓練。
- 完整模型、nullable 規則與端點表集中於 API 契約；Pydantic 位於 `backend/models.py`。Selections route 已註冊，但在 runtime state 擴充前固定回 501。
...
- 契約先於功能實作，因此 placeholder replan 不會產生可選 replan_id，selections 在完成前明確回 501，避免把未驗證候選套用成目前 Trip。

...
技術選型、contract 狀態與實作進度分開記錄；LLM 的事件解析、行程重排責任及外層 schema 已確定，provider/model 由 server-side `.env` 設定。Backend A 已完成事件 structured output／本機 fallback、replanner 注入、deterministic 說明、snapshot 與 selection 交易；B 的真正跨日重排仍待接通。Frontend owner 需依 [API 契約](../api-contract.md) 同步 types。

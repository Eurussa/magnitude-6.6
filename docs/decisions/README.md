...
技術選型、contract 狀態與實作進度分開記錄；LLM 的事件解析、行程重排責任及外層 schema 已確定。Backend B 已使用可設定的 OpenAI Responses API（預設 model `gpt-5.4-mini`）完成重排模組；事件解析、`main.py` 串接、snapshot 與 selection 交易仍未完成。Frontend owner 需依 [API 契約](../api-contract.md) 同步 types。

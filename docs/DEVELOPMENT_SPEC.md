...
事件 LLM 輸出須經 `Event.model_validate_json` 及 item/date reference 檢查，planning LLM 輸出須經對應的 Pydantic schema 與方案限制驗證；timeout、格式錯誤或無效方案應重試、使用明確標示的 fixture/fallback，或回傳 503。LLM 負責產生跨日重排行程，Python 負責 orchestration、驗證及 deterministic preference scoring。預設 `WEATHER_MODE=mock` 使用涵蓋 demo 三日的天氣 fixture；`live` 呼叫 Open-Meteo date range，失敗時明示 fallback。A 的事件 structured-output adapter 與 `agent/weather.py` 已串入多日 context，B 的 `LLMReplanner` 已完成並由 `main.py` 在 application boundary 注入。

## 目前交付範圍
已實作：health/trip/preferences/replan/selections API、完整外層 Pydantic/OpenAPI schema、多日 JSON fixture、runtime JSON、多日 weather context、前端 multi-day types、按日期分組的 Trip／Plan UI 與 Google Maps。Backend A 已完成事件 LLM structured output、本機多日 fallback parser 與明確 warning、B Protocol／503 邊界、deterministic 方案說明 fallback、snapshot 保存與 selection 原子交易；仍不開放 HTTP 任意儲存或重置 endpoint。

Backend B 的 `LLMReplanner` 已接入 HTTP Replan route；有 server-side key 時使用 planning LLM，未設定 key 或 provider／驗證失敗時使用明確標示的 planning fixture。A 會保存 ready snapshot，之後可選擇可行方案並原子套用行程與更新偏好。**尚未實作的是 A 的 LLM 推薦文字說明；現有 deterministic 說明是 fallback。** 外層契約、A 的事件解析與選擇交易、B 的跨日規劃以及前端多日流程皆已完成，未實作狀態不代表欄位仍待決定。
...
| POST `/api/replan` | `ReplanRequest` | 200 `ReplanResponse`；預設為 live／fixture ready，planner 未注入時為 placeholder；422/503 |
...
固定 A/B 介面為 `await Replanner.generate_plans(context: ReplanContext) -> PlanningResult`，Protocol 位於 `backend/contracts.py`，B 的 `LLMReplanner` 已實作此介面並由 A 的 main 注入。未注入時舊 `candidate_plans(...)` 僅產生不可選 placeholder。A 以 LLM structured output 解析 Event（失敗時本機 fallback）、組裝 context、取得天氣、保存 snapshot、補 deterministic explanation/insight 並組 response；B 擁有 planning prompt／LLM 呼叫、結構化 Plan、驗證、impact/features 與 deterministic 排序。B 不呼叫 Open-Meteo、不直接讀寫 runtime，也不依賴 `agent/`。
...
| Backend A | agent/（parser / prompts、context、weather、preference / runtime、explanation）、main.py、models.py；偏好種子、天氣 fixture、runtime JSON；snapshot / selection 交易 | structured output 驗證、Open-Meteo / fallback、時間對齊、JSON 持久化、推薦解釋、選擇冪等；維護 API 契約 |
...
必測：空字串 422、未知行程 422、事件／planning LLM 非法 JSON、planning LLM timeout、天氣 timeout、鎖定時間不變、不可行不可套用、相同輸入的偏好排序穩定、重複選擇不重複計分、再次啟動偏好仍存在。目前測試涵蓋 API 契約、事件 structured output／非法輸出／provider failure／本機 fallback、runtime JSON、天氣 context、ready snapshot、stale 拒絕與選擇冪等，以及 B 的 provider payload、structured output、重試、fallback、跨日交換、鎖定限制、changes 與偏好排序。

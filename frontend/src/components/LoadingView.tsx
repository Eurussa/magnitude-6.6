...
export function LoadingView({ message }: LoadingViewProps) {
  return (
    <main className="flex min-h-dvh flex-col bg-white px-5 pb-[calc(24px+env(safe-area-inset-bottom))] pt-[calc(28px+env(safe-area-inset-top))]">
      ...
      <section aria-live="polite" className="flex flex-1 flex-col justify-center py-8 text-center">
        <div className="mx-auto size-16 rounded-full border-[5px] border-[#dbeaf1] border-t-[#f47c57] motion-safe:animate-spin" />
        <h1 className="mt-6 text-2xl font-bold text-[#10234a]">正在比較替代方案</h1>
        ...
        <ol className="mx-auto mt-8 w-full max-w-72 space-y-4 text-left text-sm text-[#5d7187]">
          <li className="flex gap-3 text-[#173b57]"><span aria-hidden="true">●</span>理解事件與影響時間</li>
          <li className="flex gap-3"><span aria-hidden="true">○</span>檢查預約與不可移動行程</li>
          <li className="flex gap-3"><span aria-hidden="true">○</span>確認後續行程是否受影響</li>
        </ol>
      </section>
